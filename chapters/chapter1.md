...
<!--- TODO: Переместить раздел изображения, чтобы сделать порядок изложения более последовательным. Возможно будет достаточно поднять тему -->

<!--- TODO: В секцию "Текстовое форматирование" необходимо добавить информацию про элементы `u` и `pre`. Эти элементы были охвачены на занятии, но не были добавлены мной в первую редакцию. -->

<!--- TODO: Добавить информацию по атрибуту lang для <html> и добавить его в базовую структуру документа. -->
...
Формат документа указывает на то, что веб-страница состоит из содержащего HTML-код контента. Обнаружив данный Браузер сначала отформатировать перед отображением на странице.
...
Во всемирной паутине вопрос читабельности текста является очень важным, так как б**о**льшая часть информации в интернете представлена именно текстом (а не видеозаписями с котятами, как можно было бы подумать).
...
Человек по своей природе гораздо лучше усваивает информацию, если она сопровождается визуальным. Так почему бы не добавить немного изображений?
